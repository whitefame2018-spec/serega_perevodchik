...
Бот принимает ссылку на видео, скачивает аудио, делает транскрипцию через **AssemblyAI** (официальный Python SDK из GitHub, есть бесплатный tier API), переводит текст через **LibreTranslate API** и отправляет пользователю пруф для подтверждения.
